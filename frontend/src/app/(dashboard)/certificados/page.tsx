  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8 xl:p-10 space-y-8 w-full max-w-7xl mx-auto font-sans text-slate-900 print:p-0 print:m-0 print:max-w-none">
      ...
      <div className="bg-amber-50/40 p-3 sm:p-6 rounded-3xl border border-amber-200/80 shadow-lg print:bg-white print:p-0 print:border-none print:shadow-none">
        ...
        {/* Folha do Certificado Matriz */}
        <div className="relative w-full max-w-5xl mx-auto bg-[#FFFDF9] border-[12px] border-double border-[#8B0000] rounded-lg p-8 sm:p-14 shadow-2xl text-slate-900 overflow-hidden font-serif print:border-[10px] print:p-10 print:shadow-none">
          
          {/* Borda Ornamental Interna Clássica de Artes Marciais */}
          <div className="absolute inset-3 border-2 border-[#D4AF37]/80 rounded pointer-events-none"></div>
          <div className="absolute inset-5 border border-[#8B0000]/40 rounded pointer-events-none"></div>

          {/* Cantoneiras Ornamentais Tradicionais em Dourado/Vinho */}
          <div className="absolute top-6 left-6 w-12 h-12 border-t-4 border-l-4 border-[#8B0000]"></div>
          <div className="absolute top-6 right-6 w-12 h-12 border-t-4 border-r-4 border-[#8B0000]"></div>
          <div className="absolute bottom-6 left-6 w-12 h-12 border-b-4 border-l-4 border-[#8B0000]"></div>
          <div className="absolute bottom-6 right-6 w-12 h-12 border-b-4 border-r-4 border-[#8B0000]"></div>
          ...
          <div className="flex flex-col items-center justify-center text-center space-y-1 mb-8">
            <div className="w-16 h-16 rounded-full bg-[#002B7F] border-2 border-[#D4AF37] flex items-center justify-center text-white font-black text-xl shadow-md border-b-4 border-[#CE1126]">
              巴
            </div>
            <p className="text-[11px] font-black uppercase tracking-widest text-[#002B7F] font-sans">
          ...
          {/* CONTEÚDO PRINCIPAL DO CERTIFICADO: LAYOUT TRADICIONAL JAPONÊS (DIREITA) E PORTUGUÊS (ESQUERDA) */}
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch relative z-10 min-h-[460px]">
            
            {/* LADO ESQUERDO: TRADUÇÃO OFICIAL E DETALHES EM PORTUGUÊS */}
            <div className="md:col-span-6 flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-300 pb-6 md:pb-0 md:pr-8 space-y-6 font-sans">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Certificamos que o (a) karateca <strong className="text-slate-900 font-extrabold uppercase">{certData.atleta_nome_pt}</strong> cumpriu com êxito os requisitos técnicos, éticos e morais exigidos pela banca examinadora, sendo homologado (a) na graduação oficial de:
                </p>
              </div>

              {/* Caixa Destaque de Graduação */}
              <div className="p-4 bg-amber-50/80 border-2 border-[#D4AF37] rounded-2xl text-center space-y-1 my-2 shadow-xs">
                <span className="text-[10px] font-extrabold text-[#CE1126] uppercase tracking-widest block">Outorga de Faixa / Dan</span>
                <h3 className="text-lg font-black text-[#002B7F] uppercase tracking-wide">{certData.graduacao_pt}</h3>
                <p className="text-xs font-bold text-slate-700">{certData.estilo_pt}</p>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-200 text-xs">
                <div className="flex justify-between items-end gap-4">
                  <div className="space-y-1">
                    <p className="font-extrabold text-slate-900">{certData.sensei_1_nome_pt}</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Presidente FBKE / Chief Instructor GRKK</p>
                  </div>
                  <div className="space-y-1 text-right">
                    <p className="font-extrabold text-slate-900">{certData.sensei_2_nome_pt}</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Banca Examinadora / Consultor Técnico</p>
                  </div>
                </div>

                <div className="text-[10px] text-slate-500 font-mono text-center pt-2">
                  Data de Emissão: <strong>{certData.data_pt}</strong> • Autenticidade Registrada via QR Code FBKE
                </div>
              </div>
            ...
            <div className="md:col-span-6 flex flex-row-reverse justify-around items-start pt-2 px-2 text-slate-900 font-serif leading-none select-none relative min-h-[440px]">
              
              {/* COLUNA A: Número de Registro (第 二〇四七 号) */}
              <div className="flex flex-col items-center space-y-3 writing-vertical text-xs font-semibold text-slate-700">
                <span className="tracking-widest">{certData.registro_n}</span>
              </div>

              {/* COLUNA B: Título Central (證 - Certificado) */}
              <div className="flex flex-col items-center space-y-4 writing-vertical">
                <span className="text-4xl font-black tracking-widest text-slate-950 font-serif">證</span>
              </div>

              {/* COLUNA C: Nome do Atleta (右者沖繩傳 剛柔流空手道 パウロ・カサイス) */}
              <div className="flex flex-col items-center space-y-4 writing-vertical text-sm font-extrabold text-slate-900">
                <span className="text-base tracking-widest leading-loose">右者沖繩傳</span>
                <span className="text-lg font-black tracking-widest text-[#002B7F]">{certData.estilo_jp}</span>
                <span className="text-base font-black tracking-widest text-[#CE1126] py-2">{certData.atleta_nome_jp}</span>
              </div>

              {/* COLUNA D: Graduação em Kanji (二段 允許ス) */}
              <div className="flex flex-col items-center space-y-3 writing-vertical text-lg font-black text-slate-950">
                <span className="tracking-widest text-xl">{certData.graduacao_jp}</span>
              </div>

              {/* COLUNA E: Data em Kanji (一九九五年 三月三十日 / 二〇二六年 七月二十五日) */}
              <div className="flex flex-col items-center space-y-2 writing-vertical text-xs font-bold text-slate-700">
                <span className="tracking-widest">{certData.data_jp}</span>
              </div>

              {/* COLUNA F: Entidade (國際沖繩剛柔流空手道連盟) */}
              <div className="flex flex-col items-center space-y-2 writing-vertical text-xs font-extrabold text-[#002B7F]">
              ...
              <div className="flex flex-col justify-between h-full space-y-6 relative">
                ...
                <div className="flex flex-col items-center space-y-2 writing-vertical text-xs font-bold text-slate-800">
                  <span className="text-[10px] text-slate-500">{certData.sensei_2_cargo_jp}</span>
                  <span>{certData.sensei_2_nome_jp}</span>
                </div>

                <div className="flex flex-col items-center space-y-2 writing-vertical text-sm font-black text-slate-950 relative">
                  <span className="text-[10px] text-slate-500 font-semibold">{certData.sensei_1_cargo_jp}</span>
                  <span className="text-base font-black">{certData.sensei_1_nome_jp}</span>

                  {/* CARIMBO REDONDO HANKO VERMELHO SOBREPOSTO (Carimbo de Paulo Casais / Goju-Ryu) */}
                  {certData.exibir_hanko && (
                    <div className="absolute -bottom-10 -left-6 z-20">
                      <HankoOfficialSeal className="w-28 h-28 transform -rotate-12 opacity-95" />
                    </div>
                  )}
                </div>
          ...
          {/* Rodapé da Matriz */}
          <div className="mt-8 pt-4 border-t border-slate-200 flex items-center justify-between text-[9px] text-slate-400 font-sans print:mt-6">
            <span>Matriz de Certificação Oficial • GRKK / FBKE 2026</span>
      ...
      {/* Estilos Globais para Escrita Vertical em Kanji (Tate-gaki) */}
      <style jsx global>{`
        .writing-vertical {
